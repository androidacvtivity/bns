...
MAX(CASE WHEN DD.CAPITOL IN (1090) AND DD.PERIOADA= :PERIOADA    THEN NVAL(DD.COL1) ELSE 0 END) ||'<> '||
MAX(CASE WHEN D.CAPITOL  IN (1090) AND D.PERIOADA = :PERIOADA-1  THEN NVAL(D.COL2)  ELSE 0 END)
AS REZULTAT 
FROM
...
WHERE
  ...
  AND DD.CAPITOL = 1090  AND D.CAPITOL IN 1090            
...
NVAL(MAX(CASE WHEN DD.CAPITOL IN (1090)  AND DD.PERIOADA= :PERIOADA     THEN NVAL(DD.COL1) ELSE 0 END)) <> 
NVAL(MAX(CASE WHEN D.CAPITOL  IN (1090)  AND D.PERIOADA = :PERIOADA-1   THEN NVAL(D.COL2)  ELSE 0 END))
