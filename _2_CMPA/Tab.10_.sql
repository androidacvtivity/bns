...
SELECT
    ...
    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011','031','041') AND (D.PERIOADA IN (:pPERIOADA  )  AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%')  THEN D.COL2  ELSE NULL END),0) ||'~'||SUM(ROWNUM) AS NR_ROW, 
      R.NR_ROW ORDINE,
    '0002003003' AS DECIMAL_POS,
     R.NUME_ROW NUME_ROW,
     ROUND( SUM(CASE WHEN  (D.PERIOADA IN (:pPERIOADA  )) THEN NVAL(D.COL1)  ELSE 0 END) / 
            NOZERO(SUM(CASE WHEN  (D.PERIOADA IN (:pPERIOADA-1 )) THEN NVAL(D.COL1)  ELSE 0 END)),0)   AS COL1,
            
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND (D.PERIOADA IN (:pPERIOADA  )  AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%')  THEN D.COL1  ELSE NULL END),2) AS COL2,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND (D.PERIOADA IN (:pPERIOADA  )  AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  )  THEN D.COL2  ELSE NULL END),0) AS COL3,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND (D.PERIOADA IN (:pPERIOADA  )  AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN NVAL(D.COL2)/NOZERO(NVAL(D.COL1))  ELSE NULL END),2) AS COL4,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('041') AND (D.PERIOADA IN (:pPERIOADA  )  AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN D.COL1  ELSE NULL END),0) AS COL5,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('041') AND (D.PERIOADA IN (:pPERIOADA  ) AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN D.COL2  ELSE NULL END),0) AS COL6,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('041') AND (D.PERIOADA IN (:pPERIOADA  ) AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN NVAL(D.COL2)/NOZERO(NVAL(D.COL1))  ELSE NULL END),2) AS COL7,
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('031') AND (D.PERIOADA IN (:pPERIOADA  ) AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN D.COL1  ELSE NULL END),0) AS COL8,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('031') AND (D.PERIOADA IN (:pPERIOADA  ) AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%' )  THEN D.COL2  ELSE NULL END),0) AS COL9,
    ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('031') AND (D.PERIOADA IN (:pPERIOADA  ) AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  )  THEN NVAL(D.COL2)/NOZERO(NVAL(D.COL1))  ELSE NULL END),2) AS COL10         
