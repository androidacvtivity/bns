...
--    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND   THEN NVAL(D.COL18) / NOZERO(D.COL4) ELSE NULL END),2) ||'~'||VBG.RIND AS NR_ROW, 
     CC.CODUL ORDINE,
    '2222' AS DECIMAL_POS,
     CC.DENUMIRE,NUME_ROW,
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND  THEN NVAL(D.COL18) / NOZERO(D.COL5) ELSE NULL END),2) AS COL1   
