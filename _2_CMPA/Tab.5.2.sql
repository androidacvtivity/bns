SELECT 
    ...
    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND    THEN NVAL(D.COL1)  ELSE NULL END),2) ||'~'||VBG.RIND AS NR_ROW, 
     VBG.RIND ORDINE,
    '001' AS DECIMAL_POS,
     REPLACE(VBG.DENUMIRE, '<br>') NUME_ROW,
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND   THEN NVAL(D.COL13)  ELSE NULL END),2) AS COL1,   
     ROUND(
     
     (SUM(CASE WHEN MR.RIND = VBG.RIND  THEN NVAL(D.COL13)ELSE NULL END) / NOZERO(SUM(CASE WHEN MR.RIND = VBG.RIND  THEN NVAL(D.COL1)ELSE NULL END))) * 100 ,1) AS COL2
