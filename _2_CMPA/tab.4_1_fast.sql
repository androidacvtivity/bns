   ...
   SELECT 
    ...
    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND AND (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA)   THEN NVAL(D.COL18) / NOZERO(D.COL4) ELSE NULL END),2) ||'~'||VBG.RIND AS NR_ROW, 
     VBG.RIND ORDINE,
    '2222' AS DECIMAL_POS,
     REPLACE(VBG.DENUMIRE, '<br>') NUME_ROW,
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND AND  (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA)  THEN NVAL(D.COL18) / NOZERO(D.COL5) ELSE NULL END),2) AS COL1,   
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND AND  (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA) THEN NVAL(D.COL19) / NOZERO(D.COL9) ELSE NULL END ),2) AS COL2, 
     ROUND(SUM(CASE WHEN MR.RIND = VBG.RIND AND  (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA) THEN NVAL(D.COL19) / NOZERO(D.COL10) ELSE NULL END ),2) AS COL3
