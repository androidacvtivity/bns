...
--    '0' AS NUME_SECTIE2,
    ...
    -- D.UNIT_CODE,           
     ROUND( SUM(CASE WHEN 1=1 AND MR.RIND IN ('030') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL2  ELSE NULL END),0) ||'~'||SUM(ROWNUM) AS NR_ROW, 
      R.NR_ROW ORDINE,
    '00000' AS DECIMAL_POS,
     R.NUME_ROW NUME_ROW,
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('032') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL2  ELSE NULL END),0)  AS COL1,
     ROUND(
     
     SUM(CASE WHEN 1=1 AND MR.RIND IN ('030') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN NVAL(D.COL2)  ELSE NULL  END) / 
     NOZERO(SUM(CASE WHEN 1=1 AND MR.RIND IN ('032') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN NVAL(D.COL2)  ELSE  NULL END)) ,0)  AS COL2
