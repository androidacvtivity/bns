...
SELECT 
  ...
  MAX(CASE WHEN DD.PERIOADA IN (:PERIOADA) AND  DD.NUM IN (2,3,4)  THEN USER_EREPORTING.NVAL(DD.COL4) ELSE 0 END)  AS REZULTAT
 FROM 
...
WHERE
 ...
 MAX(CASE WHEN D.PERIOADA IN (:PERIOADA-1)   THEN USER_EREPORTING.NVAL(D.COL4) ELSE 0 END)  AS REZULTAT
