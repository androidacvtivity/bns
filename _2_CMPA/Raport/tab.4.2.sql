SELECT 
    ...
    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN  (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA)   THEN NVAL(D.COL18) / NOZERO(D.COL4) ELSE NULL END),2) ||'~'||MR.RIND AS NR_ROW, 
     MR.RIND ORDINE,
    '2222' AS DECIMAL_POS,
     REPLACE(MR.DENUMIRE, '<br>') NUME_ROW,
     ROUND(SUM(CASE WHEN   (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA)  THEN NVAL(D.COL18) / NOZERO(D.COL5) ELSE NULL END),2) AS COL1,   
     ROUND(SUM(CASE WHEN   (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA) THEN NVAL(D.COL19) / NOZERO(D.COL9) ELSE NULL END ),2) AS COL2, 
     ROUND(SUM(CASE WHEN   (D.PERIOADA BETWEEN FLOOR(:pPERIOADA/4)*4 AND :pPERIOADA) THEN NVAL(D.COL19) / NOZERO(D.COL10) ELSE NULL END ),2) AS COL3
...
--    SELECT 'familii de albine' AS DENUMIRE,31 AS ORDINE,  '202' AS RIND FROM DUAL 
--         ) VBG
--     
