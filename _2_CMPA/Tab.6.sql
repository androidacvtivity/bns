SELECT
    ...
    '0' AS NUME_SECTIE2,
     ...
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL1  ELSE NULL END),0) ||'~'||SUM(ROWNUM) AS NR_ROW, 
      R.NR_ROW ORDINE,
    '02' AS DECIMAL_POS,
     R.NUME_ROW NUME_ROW,
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL2  ELSE NULL END),0)  AS COL1,
     
     ROUND(SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL2  ELSE NULL END) / 
     SUM(CASE WHEN 1=1 AND MR.RIND IN ('011') AND CT.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  THEN D.COL1  ELSE NULL END),2) AS COL2
