...
SELECT
  ...
  '0' AS NUME_SECTIE2,
   ...
   NULL COL1,
   NULL COL2,
   SUM(CASE WHEN D.CAPITOL IN (1010) AND (D.PERIOADA IN(:pPERIOADA ))   THEN D.COL1 ELSE NULL END)  AS COL3,
   SUM(CASE WHEN D.CAPITOL IN (1010) AND (D.PERIOADA IN(:pPERIOADA-1 ))   THEN D.COL1 ELSE NULL END)  AS COL4
...
WHERE
 ...
 --AND D.RIND NOT IN ('-')) AND 
  
  AND D.CUATM_FULL LIKE '%'||:PCOD_CUATM||';%' 
  AND D.CUIIO IN (4818482)
