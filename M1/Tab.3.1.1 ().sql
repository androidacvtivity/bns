...
WHERE
   T.ID_MDTABLE IN (CASE WHEN T.PERIOADA <= 425 THEN 2033 ELSE 2140 END) AND
   -- T.ID_MDTABLE IN (2140) AND --TABLE 1.1.1 (*)
...
ORDER BY
  ...
  INNER JOIN (
   ...
   FROM
     TABLE_OUT T
   WHERE
    T.ID_MDTABLE IN (CASE WHEN T.PERIOADA <= 425 THEN 2046 ELSE TO_NUMBER(:pID_MDTABLE) END) AND
