...
FROM
  ...
  WHERE 
     ...
     AND D.RIND IN ('0')
            ...
            ) MR ON MR.UNIT_CODE = D.UNIT_CODE    AND MR.PERIOADA = D.PERIOADA
...
NULL  AS COL3,
NULL  AS COL4,
SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA-2) THEN  D.COL1 ELSE NULL END) AS COL5,
NULL  AS COL6,
NULL  AS COL7,
NULL  AS COL8,
NULL  AS COL9,
...
FROM
  ...
  WHERE 
     ...
     AND D.RIND IN ('0')
          ...
          INNER JOIN MD_RIND R ON D.ID_MD = R.ID_MD
WHERE
  (D.PERIOADA IN (:pPERIOADA-2,:pPERIOADA))  AND  
  (D.CUATM_FULL LIKE '%'||:pCOD_CUATM||';%') AND
  D.FORM IN (62)  AND
  D.CAPITOL IN (1117)
