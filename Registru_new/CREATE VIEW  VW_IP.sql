DROP VIEW ALL_APP.VW_IP;

/* Formatted on 27/04/2019 13:57:12 (QP5 v5.336) */
CREATE OR REPLACE FORCE VIEW  VW_IP
(
    PARAMETER,
    PARAM
)
AS
    SELECT Parameter, PARAM
      FROM (SELECT 'ACTION'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'ACTION')     PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
...
ORID'                                                Parameter,
            ...
            UNION ALL
            -------------------------------------------------------------------------------
            SELECT 'BG_JOB_ID'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'BG_JOB_ID')     PARAM
              FROM DUAL
            UNION ALL
            -------------------------------------------------------------------------------
            ...
            UNION ALL
            ------------------------------------------------------------------------------
            SELECT 'DB_DOMAIN'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'DB_DOMAIN')     PARAM
              FROM DUAL
            UNION ALL
            -------------------------------------------------------------------------------
            SELECT 'DB_NAME'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'DB_NAME')     PARAM
              FROM DUAL
            UNION ALL
            -------------------------------------------------------------------------------
            ...
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'FG_JOB_ID'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'FG_JOB_ID')     PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            ...
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'GLOBAL_UID'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'GLOBAL_UID')     PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'HOST' Parameter, SYS_CONTEXT ('USERENV', 'HOST') PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            ...
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'INSTANCE'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'INSTANCE')     PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            ...
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'IP_ADDRESS'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'IP_ADDRESS')     PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'ISDBA' Parameter, SYS_CONTEXT ('USERENV', 'ISDBA') PARAM
              FROM DUAL
            UNION ALL
            --------------------------------------------------------------------------------
            SELECT 'LANG' Parameter, SYS_CONTEXT ('USERENV', 'LANG') PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'LANGUAGE'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'LANGUAGE')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'MODULE'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'MODULE')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            ...
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'NLS_SORT'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'NLS_SORT')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            ...
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'OS_USER'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'OS_USER')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            ...
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'PROXY_USER'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'PROXY_USER')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            ...
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'SESSIONID'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'SESSIONID')     PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            SELECT 'SID' Parameter, SYS_CONTEXT ('USERENV', 'SID') PARAM
              FROM DUAL
            --------------------------------------------------------------------------------
            UNION ALL
            ...
            SELECT 'TERMINAL'                              Parameter,
                   SYS_CONTEXT ('USERENV', 'TERMINAL')     PARAM
              FROM DUAL)
     WHERE PARAM IS NOT NULL;


GRANT DELETE, INSERT, SELECT, UPDATE ON ALL_APP.VW_IP TO BACKUP;
