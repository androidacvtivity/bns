SELECT 
  TR.NUME_ROW,
  SUM(CASE WHEN D.RIND IN ('1631') AND D.COL1 BETWEEN(SUBSTR(TR.NR_ROW,0,INSTR(NR_ROW,'-')-1)) AND (SUBSTR(TR.NR_ROW,INSTR(TR.NR_ROW,'-')+1,LENGTH(TR.NR_ROW))) THEN D.COL1 ELSE 0 END) AS SUPRAFATA,
  COUNT(DISTINCT CASE WHEN D.RIND IN ('1631') AND D.COL1 BETWEEN(SUBSTR(TR.NR_ROW,0,INSTR(NR_ROW,'-')-1)) AND (SUBSTR(TR.NR_ROW,INSTR(TR.NR_ROW,'-')+1,LENGTH(TR.NR_ROW))) THEN D.CUIIO END) AS NR_INTREPRINDERI
FROM
  VW_DATA_ALL D
CROSS JOIN
 (
