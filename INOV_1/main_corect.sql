...
GROUP BY  
  ...
  CTM.NUME_ROW AS NUME_ROW, 
  NULL COL1,
  SUM(CASE WHEN C.FULL_CODE LIKE '%'||CTM.CS_CAEM2||'%' THEN D.COL1  ELSE NULL END) AS COL2,
  NULL COL3
...
GROUP BY  
  ...
  NULL COL1,
  NULL COL2,
  SUM(CASE WHEN C.FULL_CODE LIKE '%'||CTM.CS_CAEM2||'%' THEN D.COL1  ELSE NULL END) AS COL3
