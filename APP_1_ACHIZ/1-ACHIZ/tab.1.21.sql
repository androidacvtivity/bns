...
SELECT
  ...
  '0' AS NR_SECTIE2,
  '0' AS NUME_SECTIE2,
  SUM(CASE WHEN CC.FULL_CODE LIKE '%'||R.CS_CUATM||'%'  AND MR.RIND IN ('120') THEN NVAL(D.COL1) ELSE NULL END )  ||'~'||R.NR_ROW  AS NR_ROW,
  R.NR_ROW AS ORDINE,
 '0000' AS DECIMAL_POS, 
  R.NUME_ROW,

    SUM(CASE WHEN CC.FULL_CODE LIKE '%'||R.CS_CUATM||'%' AND MR.RIND IN ('121')   THEN NVAL(D.COL2) ELSE NULL  END)  AS COL1,
    SUM(CASE WHEN CC.FULL_CODE LIKE '%'||R.CS_CUATM||'%' AND MR.RIND IN ('122')   THEN NVAL(D.COL1) ELSE NULL  END)  AS COL2,
    SUM(CASE WHEN CC.FULL_CODE LIKE '%'||R.CS_CUATM||'%' AND MR.RIND IN ('123')   THEN NVAL(D.COL2) ELSE NULL  END)  AS COL3
