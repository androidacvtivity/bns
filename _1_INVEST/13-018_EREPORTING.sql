SELECT  DISTINCT
    ...
    'P.C.-Rind.'||L.COL1||', Tara. '||L.COL2||': '||L.REZULTAT ||' <> '|| R.REZULTAT  AS REZULTAT 
...
SELECT 
  ...
  SUM(DD.COL1) AS COL2,
  MAX(CASE WHEN DD.PERIOADA IN (:PERIOADA) AND  DD.NUM IN (2,3,4)  THEN USER_EREPORTING.NVAL(DD.COL2) ELSE 0 END)  AS REZULTAT
 FROM 
...
WHERE
 ...
 SUM(D.COL1) AS COL2,
 MAX(CASE WHEN D.PERIOADA IN (:PERIOADA-1)   THEN USER_EREPORTING.NVAL(D.COL2) ELSE 0 END)  AS REZULTAT
