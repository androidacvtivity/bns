...
FROM
  CIS.DATA_ALL D INNER JOIN  CIS.MD_RIND R ON R.ID_MD = D.ID_MD  
  ...
  (R.CAPITOL=:CAPITOL           OR :CAPITOL = -1) AND
  (R.CAPITOL_VERS=:CAPITOL_VERS OR :CAPITOL_VERS = -1) AND
  (R.ID_MD=:ID_MD               OR :ID_MD = -1) AND
   D.FORM = 1 AND
   R.CAPITOL = 1
   AND R.RIND NOT IN ('2600')
