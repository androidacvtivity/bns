...
((SUM(CASE WHEN B.NR_ROW LIKE '%'|| D.RIND ||'%' AND D.CAPITOL IN 1022 AND D.RIND NOT IN '22' THEN NVAL(D.COL6) ELSE 0 END)+
 ...
 (SUM(CASE WHEN B.NR_ROW LIKE '%'|| D.RIND ||'%' AND D.CAPITOL IN 1023 AND D.RIND NOT IN '22' THEN NVAL(D.COL6) ELSE 0 END)+
  ...
  SUM(CASE WHEN B.NR_ROW LIKE '%'|| D.RIND ||'%' AND D.CAPITOL IN 1023 AND D.RIND NOT IN '22' THEN NVAL(D.COL10) ELSE 0 END))) AS COL11,
...
SELECT '21.a'           AS NR_ROW, 'inclusiv: elevi cu dizabilităţi сare au nevoie de şcolarizare adaptată deficiențelor'                              AS NUME_ROW, 8.1 AS ORDINE FROM DUAL UNION  
