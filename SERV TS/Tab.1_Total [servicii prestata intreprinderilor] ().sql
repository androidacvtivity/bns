...
SELECT
  ...
  SUM(CASE WHEN D.CAEM2 LIKE B.NR_ROW||'%' AND D.PERIOADA IN :pPERIOADA-12 THEN D.COL1 ELSE 0 END) AS COL5
 ...
 (D.CUATM_FULL LIKE '%'||:pCOD_CUATM||';%') AND
  ...
  CAEM2 LIKE 'N813%') AND
   ...
   (D.PERIOADA BETWEEN FLOOR((:pPERIOADA-12)/12)*12 AND :pPERIOADA-12 OR 
     ...
     SELECT
  'TRANSPORT ?I DEPOZITARE(49+50+51+52+53)' AS NUME_ROW,
  1 AS ORDINE,
  ROUND(SUM(COL1),1) AS COL1,
  ROUND(SUM(COL2),1) AS COL2,
  ROUND(SUM(COL3),1) AS COL3,
...
WHERE
 ...
 (D.CUATM_FULL LIKE '%'||:pCOD_CUATM||';%') AND
  ...
  (D.PERIOADA BETWEEN FLOOR((:pPERIOADA-12)/12)*12 AND :pPERIOADA-12 OR 
     ...
     D.CAEM2 NOT LIKE 'N772%' AND
     D.CAEM2 NOT LIKE 'M742%'
     AND B.NR_ROW IN ('H49','H50','H51','H52','H53')
)
     )
