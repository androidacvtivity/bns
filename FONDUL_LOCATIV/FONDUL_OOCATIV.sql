...
SUM(CASE WHEN  C.FULL_CODE LIKE '%'||CR.CS_CUATM||'%'  THEN L.AM_COL89 ELSE NULL END ) AS FONDUL_LOCATIV_2021,
SUM(CASE WHEN  C.FULL_CODE LIKE '%'||CR.CS_CUATM||'%'  THEN L.AM_COL90 ELSE NULL END ) AS FONDUL_LOCATIV_2021,
SUM(CASE WHEN  C.FULL_CODE LIKE '%'||CR.CS_CUATM||'%'  THEN L.AM_COL91 ELSE NULL END ) AS FONDUL_LOCATIV_2021
...
C.FULL_CODE,
 ...
 COUNT (DISTINCT CASE WHEN FC.FORM  = 40 THEN  FC.CUIIO END)  AS   AM_COL89,
 NULL AS AM_COL90,
 NULL AS AM_COL91
...
SELECT FC.CUIIO,
              ...
              FROM M53.FORM_CUIIO  FC
                   ...
                   INNER JOIN (  SELECT CUIIO, MAX (CUIIO_VERS) CUIIO_VERS
                                   FROM M53.FORM_CUIIO
                                  WHERE FORM IN (40) AND CUIIO_VERS <= :pPERIOADA_ANUL  
                               GROUP BY CUIIO) BB
           ...
           INNER JOIN M53.RENIM R
               ON (R.CUIIO = FC.CUIIO AND R.CUIIO_VERS = FC.CUIIO_VERS)
                 ...
                 INNER JOIN M53.VW_CL_CUATM C   ON C.CODUL = FC.CUATM
...
SELECT C.CODUL,
         C.FULL_CODE,
 NULL AS AM_COL89,
 COUNT (DISTINCT CASE WHEN D.FORM  =  40 THEN  D.CUIIO END)  AS  AM_COL90,
 NULL AS AM_COL91
    ...
    FROM (SELECT DISTINCT D.CUIIO,
           ...
           WHERE D.PERIOADA = :pPERIOADA_ANUL AND D.FORM IN (40)
             
           ) D
         INNER JOIN M53.VW_CL_CUATM C ON C.CODUL = D.CUATM
