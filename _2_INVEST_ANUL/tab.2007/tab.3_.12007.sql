        ...
        SELECT 
           ...
           ( 
            ...
            SELECT 
             ...
             -----------------------------------------------------------------------------------------------------------------------
             SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA) AND MR.RIND = R.RIND   THEN CIS2.NVAL(D.COL1) ELSE 0 END) /
             CIS2.NOZERO(SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA - 1 ) AND MR.RIND = R.RIND   THEN CIS2.NVAL(D.COL1) ELSE NULL END))
              AS COL2,
            -----------------------------------------------------------------------------------------------------------------------------
              
              ROUND( CASE WHEN    R.RIND IN '100' THEN 100 ELSE 
             ( SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA) AND  MR.RIND = R.RIND   THEN CIS2.NVAL(D.COL1)) ELSE 0 END)  * 100 )  /
             CIS2.NOZERO(SUM(CASE WHEN  D.PERIOADA IN (:pPERIOADA) AND  MR.RIND IN '100'  THEN CIS2.NVAL(D.COL1)  ELSE 0 END))  END,1)  AS COL3
