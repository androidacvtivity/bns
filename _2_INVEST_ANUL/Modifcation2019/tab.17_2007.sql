SELECT
           ...
           ( 
            ...
            SELECT 
             ...
             -----------------------------------------------------------------------------------------------------------------------------
              SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA) AND R.RIND =  MR.RIND   THEN CIS2.NVAL(D.COL1)+CIS2.NVAL(D.COL2) ELSE 0 END)  /
             CIS2.NOZERO(SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA-1)   AND R.RIND =  MR.RIND  THEN CIS2.NVAL(D.COL1)+CIS2.NVAL(D.COL2) ELSE 0 END)) AS COL2,
             
             -------------------------------------------------------------
             
             
             ROUND( CASE WHEN    R.RIND IN '100' THEN 100 ELSE 
             ( SUM(CASE WHEN D.PERIOADA IN (:pPERIOADA) AND R.RIND NOT IN  ('730','740','750','760','520','510','530') AND MR.RIND = R.RIND   THEN CIS2.NVAL(D.COL1)+(CIS2.NVAL(D.COL2)) ELSE 0 END)  * 100 )  /
