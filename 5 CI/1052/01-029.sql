...
--CAP.STOC Col.1 = Col.2 (trim-1)
  'RIND '||D.RIND||' - col.1 : '||
  CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (1) AND D.PERIOADA IN (:PERIOADA)   THEN  NVAL(D.COL1) ELSE NULL END)) ||' <> : col.2 - '||
  CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (1) AND D.PERIOADA IN (:PERIOADA-1)  THEN  NVAL(D.COL2) ELSE NULL END))   
  AS REZULTAT
