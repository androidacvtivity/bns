...
SELECT DISTINCT
  ...
  D.CUIIO,
  SUM(CASE WHEN D.RIND IN('421') THEN CIS2.NVAL(D.COL1) ELSE 0 END) 
...
WHERE
  ...
  D.FORM||'.'||D.CAPITOL||'.'||D.RIND IN ('4.320.421') 
  GROUP BY 
  D.CUIIO
  ...
  (
   ...
   SELECT DISTINCT
     D.CUIIO,
   SUM(CASE WHEN D.RIND IN('890') AND P.NUM IN (4) THEN CIS2.NVAL(D.COL1) ELSE 0 END) 
