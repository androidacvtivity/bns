...
SELECT DISTINCT
  ...
  D.CUIIO,
  SUM(CASE WHEN D.RIND IN('413') THEN CIS2.NVAL(D.COL1) ELSE 0 END) 
...
WHERE
  ...
  D.FORM||'.'||D.CAPITOL||'.'||D.RIND IN ('4.320.413') 
  GROUP BY 
  D.CUIIO
  ...
  (
   ...
   SELECT DISTINCT
     D.CUIIO,
   SUM(CASE WHEN D.RIND IN('862','863') AND P.NUM IN (4) THEN CIS2.NVAL(D.COL1) ELSE 0 END) 
